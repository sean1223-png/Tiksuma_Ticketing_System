...
// Check if user owns the ticket or is IT staff
$user_type_sql = "SELECT user_type FROM users WHERE username = ?";
$stmt = $conn->prepare($user_type_sql);
if (!$stmt) {
...
$user_type = $user_type_result->fetch_assoc()['user_type'] ?? 'User';
$stmt->close();

// Check if user owns the ticket
$ownership_sql = "SELECT username FROM tickets WHERE ticket_id = ?";
$stmt = $conn->prepare($ownership_sql);
$stmt->bind_param("i", $ticket_id);
$stmt->execute();
$ownership_result = $stmt->get_result();
$ticket_owner = $ownership_result->fetch_assoc()['username'] ?? '';
$stmt->close();

// Allow access if user owns ticket or is IT staff
if ($_SESSION['username'] != $ticket_owner && $user_type != 'Admin' && $user_type != 'IT Staff') {
    die("You don't have permission to view this ticket.");
}

// Update status if form is submitted with proper validation
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['status_id']) && ($user_type == 'Admin' || $user_type == 'IT Staff')) {
...
$sql = "SELECT t.ticket_id, t.subject, t.description, t.created_at,
...
// Fetch all statuses for dropdown (only for IT staff)
...
$attachments_sql = "SELECT * FROM tickets_attachment WHERE ticket_id = ?";
$stmt = $conn->prepare($attachments_sql);
if (!$stmt) {
    die("Prepare failed: " . $conn->error);
}
$stmt->bind_param("i", $ticket_id);
...
// Update ticket as viewed for IT staff
if ($user_type == 'Admin' || $user_type == 'IT Staff') {
...
<!-- Simple navigation -->
<div style="background: #343a40; color: white; padding: 15px;">
    <div style="max-width: 1200px; margin: 0 auto; display: flex; justify-content: space-between; align-items: center;">
        <h2 style="margin: 0;">TIKSUMA - Ticket System</h2>
        <div>
            <span>Welcome, <?= htmlspecialchars($_SESSION['username']) ?></span>
            <a href="it-tickets.php" style="color: white; margin-left: 20px; text-decoration: none;">
...
<div class="ticket-container">
    ...
    <?php if (($user_type == 'Admin' || $user_type == 'IT Staff')): ?>
